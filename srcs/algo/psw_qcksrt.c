...
/*   Updated: 2021/09/26 12:35:49 by wetieven         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	qcksrt_b(t_game *game, size_t high, size_t low, t_step step)
{
	...
	med = low + (high - low) / 2;
	to_sort = high - med;
	while (game->b.load && to_sort)
	{
		if (game->b.stk[game->b.load - 1].key > med)
		{
			psh(game, A);
			to_sort--;
		}
	...
	while (game->b.load && game->b.stk[0].key >= low && low != 0)
		rrot(game, B);
...
void	qcksrt_a(t_game *game, size_t low, size_t high, t_step step)
{
	...
	while (game->a.load && to_sort && !sorted(game, A, 0))
	{
		if (game->a.stk[game->a.load - 1].key <= (low + (high - low) / 2))
		{
			psh(game, B);
			to_sort--;
			if (game->b.stk[game->b.load - 1].key == (low + (high - low) / 2)
				&& to_sort > 1 && game->b.load >= 2)
				rot(game, B);
		}
		else
			rot(game, A);
	}
	if (game->b.load && game->b.stk[0].key == (low + (high - low) / 2))
		rrot(game, B);
	while (game->a.load
		&& game->a.stk[0].key <= high && high != game->info.qty - 1)
		rrot(game, A);
	nxtsrt(game, low, high, step);
}
